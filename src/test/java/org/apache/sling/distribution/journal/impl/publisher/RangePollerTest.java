...
public class RangePollerTest {
    ...
    public void testRangeAvailable() throws Exception {
        RangePoller poller =
                new RangePoller(clientProvider, TOPIC, MIN_OFFSET, MAX_OFFSET, RangePoller.DEFAULT_SEED_DELAY_SECONDS);
        ...
        PackageMessage msg2 =
                createMessage(ReqType.TEST, MIN_OFFSET + 1); // Should not be fetched as it is a test message
    ...
    public void testRangeNotYetAvailable() throws Exception {
        ...
        // We expect the poller to wait for messages to come in and then send a single seeding message which should let
        // the poller succeed
